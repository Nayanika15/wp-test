...
/**
 * Check working of filter hooks
 * If user is logged in show loggout link
 *
 */
...
/**
 * Adding view count meta field for post
 */
function wp_add_view_count_to_post($post_id){
	add_post_meta( $post_id, '_wp_post_view', 0, true);
}
add_action('wp_insert_post', 'wp_add_view_count_to_post');

/**
 * Updating view count for post
 */
function wp_increase_post_view($title, $post_id)
{	
	if(is_single($post_id))
	{	
		$view = (int)get_post_meta($post_id, '_wp_post_view', true);
		update_post_meta( $post_id, '_wp_post_view', $view);
		$title = $title .'(views count-'. $view .')';
	}

	return $title;
}

add_filter('the_title', 'wp_increase_post_view', 10, 2);
...
function add_post_view_count_column( $columns ) {
 	$column_meta = array( 'meta' => 'Views Count' );
	$columns = array_slice( $columns, 0, 4, true ) + $column_meta + array_slice( $columns, 4, NULL, true );
	return $columns;
}
add_filter( 'manage_edit-post_columns', 'add_post_view_count_column');
 ...
 */
function views_count_column( $column ) {
	global $post;
	if ( $column == 'meta' ) {
			$metaData = get_post_meta( $post->ID, '_wp_post_view', true );
			echo $metaData;
	}
}
add_action( 'manage_posts_custom_column' , 'views_count_column' );
